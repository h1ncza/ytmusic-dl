...
cya=$(tput setaf 6)
cnc=$(tput sgr0)
pipth="$(python -m site --user-base)/bin/pyinstaller"
...
$pipth --distpath='./bin' --collect-all ytmusicapi --collect-all requests --collect-all colorama --add-binary 'yt-dlp-bin:.' --onefile --name ytmusic-dl "./src/"$vrs
rm -f yt-dlp-bin
rm -f *.spec
rm -r build
